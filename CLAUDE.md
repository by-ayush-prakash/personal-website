...
- **Both feeds return full history.** Substack (`https://ayushprakash.substack.com/feed`) and Anchor (`https://anchor.fm/s/4f9f9cb0/podcast/rss`) are the complete archive sources. Verified 30 July 2026. Pull both at build time into local content files and commit those files, so the site still builds if a feed is down.
